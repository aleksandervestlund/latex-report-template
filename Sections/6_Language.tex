...
Cross\-/referencing is done using the \texttt{cleveref} package, which automatically includes the type of reference (e.g., figure, algorithm, listing) in the citation. For example, \cref{lst:fibonacci} shows how to calculate Fibonacci numbers while \cref{fig:theway} displays an image. Algorithms and procedures are referenced similarly, such as \cref{alg:expBySquaring} and \cref{pro:mult}. \Cref{fig:theway,fig:subfigures,lst:csharpMath,lst:mathMinted,lst:matlabHelloWorld,tab:budget,tab:standard}.
...
The \texttt{glossaries} package is useful for defining acronyms. Here are some examples: \glspl{api}, \gls{os}, \gls{dr}, \gls{etc}. The results can be seen after the appendix.
