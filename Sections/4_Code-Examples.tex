...
This section demonstrates how to include source code using the \mintinline{latex}{minted} package, which provides syntax highlighting for a wide range of languages. Code is typically presented in a \mintinline{latex}{listing} environment and can be customised with captions and labels for cross\-/referencing. The \mintinline{latex}{minted} package also supports inputting external files and allows for inline maths, though care must be taken when combining code and mathematical symbols.
...
# This is an example. The next line is 76 (74 + 2) characters long:
...
test
    \end{minted}
    \caption{Using math inside \mintinline{latex}{minted} environments.}
