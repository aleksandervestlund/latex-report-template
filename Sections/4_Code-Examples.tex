...
This section demonstrates how to include source code using the \latexinline{minted} package, which provides syntax highlighting for a wide range of languages. Code is typically presented in a \mintinline{latex}{listing} environment and can be customised with captions and labels for cross\-/referencing. The \latexinline{minted} package also supports inputting external files and allows for inline maths, though care must be taken when combining code and mathematical symbols.
...
test
    \end{pythoncode}
    \caption{Using math inside \latexinline{minted} environments.}
