...
Variance:
\begin{equation*}
    \Var\dellr{X} = \sigmasq\dellr{X} = \expect{\dell{X - \expect{X}}^2} = \sum_{x \in \mathcal{X}} \dell{x - \expect{X}}^2 \cdot \prob{X = x}
\end{equation*}

Some commonly used mathematical sets and transforms:
\begin{equation*}
    \NN \subset \ZZ \subset \QQ \subset \RR \subset \CC, \quad \laplaceinv{\laplace{x}} = x, \quad \fourierinv{\fourier{x}} = x, \quad \compl A
\end{equation*}
